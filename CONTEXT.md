...
- **Frame typing width**: Unresized frames use `width: max-content` + `height: fit-content` with ProseMirror `data-single-line` nowrap so the **frame hugs content** and a block runs until **Enter**. Editor + each block use `width: 100%` (`min-width: max-content` on PM) so empty/short lines span the full frame content row for I-bar clicks; resized frames use `w-full` contentFit (not `w-max`). Clicks in frame padding resolve the block by Y. RF node `width`/`height`/`style` sync from the panel box (ResizeObserver) so selection/resize chrome matches. Corner-drag sets `isUserResized` + persisted `resizeDimensions` (wrap in fixed box).
...
- **Threads / connection UX (Miro, locked split)**: **Connection point** = invisible RF Handle on the frame edge (`tt-connection-point`, geometry + snap; never painted). **Connection indicator** = plain outer DOM blue dot (`ConnectionIndicator`) that arms RF store connection from the edge handle id. Settled paths meet the **frame edge**. Thread style toolbar: **Smooth** (Bezier), **Sharp** (ridged with rounded elbows), **Linear** (straight). Do not make the indicator a second Handle.
...
- **Frame lock**: Default **locked** (`metadata.frameUnlocked: false`). Locked + content → **proportional resize only** (`keepAspectRatio` + `frameScale`) and the frame **hugs scaled text** (`resizeDimensions` = intrinsic × `frameScale` + border; padding lives **inside** the scaled wrapper + layout spacer so CSS `scale` still occupies space). Relock snaps to that box. Typing while locked+resized re-hugs. Unlock keeps **current visual size** (`frameScale` unchanged; snapshot `resizeDimensions`). Unlocked: free resize; if frame < scaled content, **inner overflow hidden** + **ChevronDown** expands to content (caret **always visible** when clipped/expanded — not hover-gated; selected order: rotate → caret → lock → wrap; unselected caret keeps the same x via `marginLeft: 26`); arrow flips to **ChevronUp** and restores `collapsedFrameSize`. Unlocked chrome also shows **Wrap text** (`WrapText`, right of lock) → `metadata.frameTextWrap`: soft-wrap in the frame width, height hugs wrapped content (no clip/chevron). Unlocked non-wrap also **shrinks height** when the box is taller than content (kills empty bottom slack after pad/content changes; never grows into an un-clip). Relock clears wrap. **Rotate / wrap / lock** chrome only when the frame is **selected** (not on unselected hover). Frame content vertical pad is **equal 4px** top/bottom. Persist `frameUnlocked`, `frameTextWrap`, `frameScale`, `resizeDimensions`, `collapsedFrameSize`. Block line-height stays **1.7** (height-based line-height broke lock-to-text).
