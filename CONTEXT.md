...
- **Block selection chrome** (selected blocks): **connected blue rectangle** (`NodeResizeControl` line variant on top/right/bottom/left) meeting **4 white circular corner** handles (not the rounded card border) + **solid dark mid-side** connection nodules **outside** the rectangle (`±14px` via CSS `!important` + matching `Handle` style; drag thread uses `PointerConnectionLine` so the free end tracks the cursor) + bottom-left chrome: **Lock** (only if the frame has content) · **RotateCw** (`metadata.rotation`; Shift snaps 15°) · **ChevronDown / ChevronUp** when unlocked overflow / expanded. Hover shows chrome even when unselected. CSS in `globals.css` under `.react-flow__node-chatPanel`.
- **Frame lock**: Default **locked** (`metadata.frameUnlocked: false`). Locked + content → **proportional resize only** (`keepAspectRatio` + `frameScale`). Unlock keeps **current visual size** (`frameScale` unchanged; snapshot `resizeDimensions`). Unlocked: free resize; if frame < scaled content, **inner overflow hidden** + **ChevronDown** expands to content; arrow flips to **ChevronUp** and restores `collapsedFrameSize`. Persist `frameUnlocked`, `frameScale`, `resizeDimensions`, `collapsedFrameSize`.
...
- **Block groups**: visual container RF node `blockGroup` (`metadata.isBlockGroup`); children use RF `parentId` + `metadata.blockGroupId`. Frame has no ⋮⋮; content blocks inside each child card do. Group-level multi-action deferred.
