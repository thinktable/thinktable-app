...
- **Threads / connection UX (Miro, locked split)**: **Connection point** = invisible RF Handle on the frame edge (`tt-connection-point`, geometry + snap; never painted). **Connection indicator** = plain outer DOM blue dot (`ConnectionIndicator`) that arms RF store connection from the edge handle id. Settled paths meet the **frame edge** (`connectionPointOnNode`). Do not make the indicator a second Handle.
