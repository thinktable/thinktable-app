...
- Empty board/canvas **left-click** (no panel selected) opens an **add block** menu at the click (`board-flow` `addBlockMenu`) with Block / Flashcard; blocks spawn at click flow coords via `metadata.position`. Deselect click does not open the menu; Escape / outside click dismisses. Double-click still places the I-bar for type-to-create blocks.
...
- **Block selection chrome** (selected blocks): blue border + **4 white circular corner** `NodeResizeControl`s (explicit `metadata.resizeDimensions` box resize; RF node w/h synced from measured DOM so left/top corners don’t look like pure moves) + **solid dark mid-side** connection handles + **RotateCw** handle bottom-left (`metadata.rotation` degrees; Shift snaps 15°). CSS in `globals.css` under `.react-flow__node-chatPanel`.
