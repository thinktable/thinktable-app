...
- Minimap + Free nav menu sit on the **bottom-left** of the page (left edge aligns with top-bar menu button at 8px; bottom aligns with chat brand icon at 8px; no prompt-driven jump): left **Scroll/Zoom** select (label shows the active wheel mode), right **zoom %** (`nav-zoom-control.tsx`, moved from the top-bar toolbar). Linear toggle removed (always Free/canvas). The **chat sidebar toggle logo** (with topper) stays on the **bottom-right** of the page column; the **flashcard study menu** (`left-vertical-menu.tsx`) + its open/close pill sit **centered at the bottom** of the page.
